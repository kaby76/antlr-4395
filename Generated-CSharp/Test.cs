...
{
    public static ExprParser Parser { get; set; }
...
//            ParserATNSimulator.trace_atn_sim = true;
        ...
        var mv = new MyExprVisitor();
        mv.Visit(tree);

        if (show_profile)
        {
            System.Console.Error.WriteLine(String.Join(",\n\r",
                parser.ParseInfo.getDecisionInfo().Select(d => d.ToString())));
        }
...
//    }
    ...
    {
        System.Console.WriteLine("VisitDecl");
        return this.VisitChildren(context);
    }

    public override int VisitExpr(ExprParser.ExprContext context)
    {
        System.Console.WriteLine("VisitExpr");
        return this.VisitChildren(context);
    }
}